...
All boards 20x20, every clue number read correctly, and each board has
...
solved 0.39 s — carries the only `8` in the set.

![solved](nonogram_3378_solved_t20260813.png)

![overlay](nonogram_3378_overlay_t20260813.png)
...
so nothing downstream could contradict it. `IMG_3378` then showed the mirror
case: `8` had no real template at all, so `2 ? ? 2 8` read as `2 ? ? 2 3`.
The bank is now harvested from all three screenshots with no system-font
fallback, and all three boards are unique.
